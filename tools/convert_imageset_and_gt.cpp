...
public:
    ...
    ImagePair(std::string line)
    {
        clrptr();
        stringstream s(line);
        s >> img1_filename >> img2_filename >> gt_filename;
    }
    ~ImagePair()
    {
        if(data) delete data;
    ...
    void read_data()
    {
        ...
        cv_gt = cv::imread(gt_filename, CV_LOAD_IMAGE_COLOR);
        ...
        if (!cv_gt.data) LOG(FATAL) << "Could not open or find file " << gt_filename;
        ...
        assert(cv_gt.cols==xSize && cv_gt.rows==ySize );
        width=xSize;
        height=ySize;
        ...
        for (int c = 0; c < 3; ++c)
            for (int y = 0; y < height; ++y)
                for (int x = 0; x < width; ++x) {
                    unsigned char value=cv_gt.at<cv::Vec3b>(y, x)[c];
                    *(ptr++)=value;
                }
        assert(ptr==data+3*3*width*height);
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
   ...
   // sequential
    snprintf(key_cstr, kMaxKeyLength, "%08d_%s", line_id,
        lines[line_id].gt_filename.c_str());
    string value;
