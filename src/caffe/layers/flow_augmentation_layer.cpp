...
void FlowAugmentationLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top)
{
  CHECK_EQ(bottom.size(), 3) << "Flow augmentation layer has three input blobs:GT, Img1TransfParams, Img2TransfParams";
  CHECK_EQ(top.size(), 1) << "Flow augmentation layer outputs one output blob: Augmented GT";
  ...
  CHECK_EQ(channels, 1) << "gt data must have one channel";
